...
export default function OSTopicViewer() {
  ...
  const handleMarkComplete = async () => {
    try {
      await axios.post('/api/interview/os/progress', { topicId: id }, { withCredentials: true });
      fetchData();
    } catch (error) {
  ...
  const handleQuizSubmit = async () => {
    ...
    try {
      await axios.post('/api/interview/os/quiz', { topicId: id, score }, { withCredentials: true });
      setQuizSubmitted(true);
      setQuizScore(score);
      if (score >= topicData.quiz.passingScore) {
        handleMarkComplete();
      }
    } catch (error) {
  ...
  const toggleBookmark = async (type, itemId) => {
    try {
      await axios.post('/api/interview/os/bookmark', { type, itemId }, { withCredentials: true });
      fetchData();
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
        ...
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-bold text-lg hidden sm:block flex items-center gap-2">
              <FiTerminal className="text-emerald-500 inline mr-2" />
              {topic.title}
            </h1>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-6 flex gap-8 border-t border-white/5 pt-2 overflow-x-auto">
          <button
            ...
            className={`pb-4 px-2 font-medium text-sm transition-colors border-b-2 whitespace-nowrap ${
              activeTab === 'notes' ? 'border-emerald-500 text-emerald-400' : 'border-transparent text-gray-400 hover:text-gray-200'
          ...
          <button
            ...
            className={`pb-4 px-2 font-medium text-sm transition-colors border-b-2 whitespace-nowrap ${
              activeTab === 'questions' ? 'border-emerald-500 text-emerald-400' : 'border-transparent text-gray-400 hover:text-gray-200'
            } flex items-center gap-2`}
          >
            <FiHelpCircle /> Interview Questions
            <span className="bg-white/10 text-xs py-0.5 px-2 rounded-full">{questions?.length || 0}</span>
          </button>
          {topicData.quiz && (
            <button
              onClick={() => setActiveTab('quiz')}
              className={`pb-4 px-2 font-medium text-sm transition-colors border-b-2 whitespace-nowrap ${
                activeTab === 'quiz' ? 'border-emerald-500 text-emerald-400' : 'border-transparent text-gray-400 hover:text-gray-200'
              } flex items-center gap-2`}
            >
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        {activeTab === 'notes' && topicData.notes && (
          <div className="prose prose-invert max-w-none prose-headings:text-[var(--color-primary)] prose-h1:text-4xl prose-h2:text-3xl prose-h3:text-2xl prose-p:text-gray-300 prose-p:leading-relaxed prose-a:text-[var(--color-primary)] hover:prose-a:text-[var(--color-secondary)] prose-strong:text-white prose-ul:text-gray-300 prose-li:marker:text-[var(--color-primary)]">
            <ReactMarkdown>{topicData.notes.content}</ReactMarkdown>
            
            {topicData.notes.sections && topicData.notes.sections.map((sec, idx) => (
              <div key={idx} className="mt-8">
                <h3 className="text-xl font-bold text-emerald-300 mb-3">{sec.title}</h3>
                ...
                return (
                  <div key={q._id} className="bg-[#111] border border-white/10 rounded-2xl p-6 relative group">
                    <button 
                      ...
                      className="absolute top-6 right-6 text-gray-500 hover:text-emerald-400 transition-colors"
                    >
                      <FiBookmark className={isBookmarked ? 'fill-emerald-500 text-emerald-500' : ''} size={20} />
                    </button>
        ...
        {activeTab === 'quiz' && topicData.quiz && (
          <div className="space-y-8">
            {quizSubmitted ? (
              <div className={`p-8 rounded-2xl border text-center ${
                quizScore >= topicData.quiz.passingScore 
                  ? 'bg-green-900/20 border-green-500/30' 
                  : 'bg-red-900/20 border-red-500/30'
              }`}>
                <h2 className="text-3xl font-bold mb-2">
                  {quizScore >= topicData.quiz.passingScore ? 'Quiz Passed!' : 'Needs More Revision!'}
                </h2>
                <p className="text-xl mb-6">You scored {quizScore}%</p>
                <div className="text-sm text-gray-400">Passing score is {topicData.quiz.passingScore}%</div>
              ...
              <>
                <div className="bg-emerald-900/20 border border-emerald-500/30 p-6 rounded-2xl mb-8">
                  <h3 className="font-bold text-xl mb-2">{topicData.quiz.title}</h3>
                  <p className="text-emerald-200">Validate your knowledge. Passing score: {topicData.quiz.passingScore}%</p>
                </div>

                {topicData.quiz.questions.map((q, qIndex) => (
                      ...
                      {q.options.map((opt, oIndex) => (
                        ...
                        >
                          ...
                          <div className={`w-5 h-5 rounded-full border-2 mr-4 flex items-center justify-center ${
                            quizAnswers[qIndex] === oIndex ? 'border-emerald-500' : 'border-gray-500'
                          }`}>
                            {quizAnswers[qIndex] === oIndex && <div className="w-2.5 h-2.5 rounded-full bg-emerald-500" />}
                          </div>
                          <span>{opt}</span>
                        </label>
                      ))}
                ...
                <button 
                  ...
                  disabled={Object.keys(quizAnswers).length !== topicData.quiz.questions.length}
                  className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 disabled:bg-gray-800 disabled:text-gray-500 rounded-xl font-bold transition-colors"
                >
                  Submit Quiz
                </button>
              </>
