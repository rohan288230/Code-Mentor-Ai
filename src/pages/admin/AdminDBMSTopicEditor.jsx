...
export default function AdminDBMSTopicEditor() {
  const { id } = useParams();
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      if (activeTab === 'notes') {
        const res = await apiClient.get(`/admin/dbms/topic/${id}/notes`, { withCredentials: true });
        setNotesContent(res.data.content || '');
      } else if (activeTab === 'questions') {
        const res = await apiClient.get(`/admin/dbms/topic/${id}/questions`, { withCredentials: true });
        setQuestions(res.data);
      } else if (activeTab === 'quiz') {
        const res = await apiClient.get(`/admin/dbms/topic/${id}/quiz`, { withCredentials: true });
  ...
  const handleSaveNotes = async () => {
    try {
      setSaving(true);
      await axios.post(`/api/admin/dbms/topic/${id}/notes`, { content: notesContent, sections: [] }, { withCredentials: true });
  ...
  const handleAddQuestion = async (e) => {
    ...
    try {
      ...
      await axios.post(`/api/admin/dbms/topic/${id}/question`, {
        ...newQuestion,
        companyTags: tags
  ...
  const handleDeleteQuestion = async (qId) => {
    ...
    try {
      await axios.delete(`/api/admin/dbms/question/${qId}`, { withCredentials: true });
      fetchData();
    } catch (error) {
  ...
  const handleSaveQuiz = async () => {
    try {
      setSaving(true);
      await axios.post(`/api/admin/dbms/topic/${id}/quiz`, quiz, { withCredentials: true });
    ...
    <div>
      ...
      <div className="flex gap-4 border-b border-white/10 mb-8">
        ...
        <button
          ...
          className={`pb-4 px-4 font-medium transition-colors border-b-2 ${
            activeTab === 'questions' ? 'border-blue-500 text-blue-400' : 'border-transparent text-gray-400 hover:text-white'
          }`}
        >
          Interview Questions
        </button>
        <button
          ...
          className={`pb-4 px-4 font-medium transition-colors border-b-2 ${
            activeTab === 'quiz' ? 'border-blue-500 text-blue-400' : 'border-transparent text-gray-400 hover:text-white'
          }`}
        >
          Practice Quiz
        </button>
            ...
            <div className="flex flex-col">
              <div className="flex justify-between items-center mb-4">
                ...
                <button
                  ...
                  disabled={saving}
                  className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 px-4 py-2 rounded-lg font-semibold transition-colors text-sm"
                >
              ...
              <textarea
                ...
                className="flex-1 w-full bg-[#111] border border-white/10 rounded-xl p-4 focus:outline-none focus:border-blue-500 font-mono text-sm resize-none"
                placeholder="# Enter markdown content here..."
              />
            </div>
            <div className="flex flex-col">
              <label className="font-semibold text-gray-300 mb-4 block">Live Preview</label>
              <div className="flex-1 bg-[#111] border border-white/10 rounded-xl p-6 overflow-y-auto prose prose-invert prose-blue max-w-none">
              ...
              <form onSubmit={handleAddQuestion} className="bg-[#111] p-6 rounded-xl border border-white/10 mb-8">
                ...
                <div className="mb-4">
                  ...
                  <textarea
                    ...
                    className="w-full bg-black/50 border border-white/10 rounded-lg p-2.5 focus:outline-none focus:border-blue-500 h-32"
                    required
                  />
                </div>
        ...
        {activeTab === 'quiz' && (
          <div className="max-w-3xl">
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#111] border border-white/10 rounded-lg p-2.5 focus:outline-none focus:border-blue-500"
                />
              </div>
              <div>
                ...
                <div key={qIndex} className="bg-[#111] p-6 rounded-xl border border-white/10 relative">
                  ...
                  <div className="mb-4 pr-8">
                    ...
                    <input
                      ...
                      className="w-full bg-black/50 border border-white/10 rounded-lg p-2 focus:outline-none focus:border-blue-500"
                    />
                  </div>
                      ...
                      <div key={oIndex} className="flex items-center gap-3">
                        ...
                        <input
                          ...
                          className={`flex-1 bg-black/50 border rounded-lg p-2 focus:outline-none ${
                            q.correctAnswerIndex === oIndex ? 'border-blue-500' : 'border-white/10 focus:border-gray-500'
                          }`}
            ...
            <button
              ...
              className="mt-6 w-full py-4 border-2 border-dashed border-white/20 hover:border-blue-500 text-gray-400 hover:text-blue-400 rounded-xl transition-colors font-semibold flex items-center justify-center gap-2"
            >
