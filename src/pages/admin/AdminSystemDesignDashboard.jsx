...
import { FiPlus, FiEdit2, FiTrash2, FiServer } from 'react-icons/fi';

export default function AdminSystemDesignDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error('Error fetching admin System Design data:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddTopic = async (e) => {
    ...
    try {
      await axios.post('/api/admin/system-design/topic', {
        ...newTopic,
  ...
  const handleDeleteTopic = async (id) => {
    ...
    try {
      await axios.delete(`/api/admin/system-design/topic/${id}`, { withCredentials: true });
      fetchData();
    } catch (error) {
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-3">
            <FiServer className="text-green-500" /> System Design Editor
          </h1>
          <p className="text-gray-400 mt-2">Manage architecture topics, interview questions, and quizzes.</p>
        </div>
        <button
          ...
          <form onSubmit={handleAddTopic} className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-black/50 border border-white/10 rounded-lg p-2.5 focus:outline-none focus:border-green-500 h-24"
                required
              />
            </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {topics.map(topic => (
          <div key={topic._id} className="bg-[#111] border border-white/5 p-6 rounded-xl flex items-center justify-between group hover:border-white/20 transition-all">
            <div className="flex items-center gap-6">
              <div className="w-12 h-12 rounded-lg bg-green-500/10 flex items-center justify-center text-green-400 font-bold text-xl">
                {topic.order}
              </div>
              <div>
            ...
            <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
              <button 
                onClick={() => navigate(`/admin/system-design/topic/${topic._id}`)}
        ...
        {topics.length === 0 && !isAddingTopic && (
          <div className="text-center py-12 bg-[#111] rounded-xl border border-white/5">
            <FiServer className="text-4xl text-gray-600 mx-auto mb-3" />
