...
import { FiArrowLeft, FiCheckCircle, FiBookOpen, FiHelpCircle, FiEdit3, FiBookmark, FiDatabase } from 'react-icons/fi';
import ReactMarkdown from 'react-markdown';

export default function DBMSTopicViewer() {
  const { id } = useParams();
  ...
  const fetchData = async () => {
    try {
      ...
      const [topicRes, progressRes] = await Promise.all([
        axios.get(`/api/interview/dbms/topic/${id}`, { withCredentials: true }),
  ...
  const handleMarkComplete = async () => {
    try {
      await axios.post('/api/interview/dbms/progress', { topicId: id }, { withCredentials: true });
      fetchData();
    } catch (error) {
  ...
  const handleQuizSubmit = async () => {
    ...
    try {
      await axios.post('/api/interview/dbms/quiz', { topicId: id, score }, { withCredentials: true });
      setQuizSubmitted(true);
      setQuizScore(score);
  ...
  const toggleBookmark = async (type, itemId) => {
    try {
      await axios.post('/api/interview/dbms/bookmark', { type, itemId }, { withCredentials: true });
      fetchData();
    } catch (error) {
        ...
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-bold text-lg hidden sm:block flex items-center gap-2">
              <FiDatabase className="text-blue-500 inline mr-2" />
              {topic.title}
            </h1>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-6 flex gap-8 border-t border-white/5 pt-2 overflow-x-auto">
          <button
            ...
            className={`pb-4 px-2 font-medium text-sm transition-colors border-b-2 whitespace-nowrap ${
              activeTab === 'notes' ? 'border-blue-500 text-blue-400' : 'border-transparent text-gray-400 hover:text-gray-200'
            } flex items-center gap-2`}
          >
            <FiBookOpen /> DBMS Notes
          </button>
          <button
            onClick={() => setActiveTab('questions')}
            className={`pb-4 px-2 font-medium text-sm transition-colors border-b-2 whitespace-nowrap ${
              activeTab === 'questions' ? 'border-blue-500 text-blue-400' : 'border-transparent text-gray-400 hover:text-gray-200'
            } flex items-center gap-2`}
          >
          ...
          {topic.quiz && (
            <button
              ...
              className={`pb-4 px-2 font-medium text-sm transition-colors border-b-2 whitespace-nowrap ${
                activeTab === 'quiz' ? 'border-blue-500 text-blue-400' : 'border-transparent text-gray-400 hover:text-gray-200'
              } flex items-center gap-2`}
            >
          ...
          <div className="prose prose-invert max-w-none prose-headings:text-[var(--color-primary)] prose-h1:text-4xl prose-h2:text-3xl prose-h3:text-2xl prose-p:text-gray-300 prose-p:leading-relaxed prose-a:text-[var(--color-primary)] hover:prose-a:text-[var(--color-secondary)] prose-strong:text-white prose-ul:text-gray-300 prose-li:marker:text-[var(--color-primary)]">
            ...
            {topic.notes.sections && topic.notes.sections.map((sec, idx) => (
              <div key={idx} className="mt-8">
                <h3 className="text-xl font-bold text-blue-300 mb-3">{sec.title}</h3>
                ...
                return (
                  <div key={q._id} className="bg-[#111] border border-white/10 rounded-2xl p-6 relative group">
                    <button 
                      ...
                      className="absolute top-6 right-6 text-gray-500 hover:text-blue-400 transition-colors"
                    >
                      <FiBookmark className={isBookmarked ? 'fill-blue-500 text-blue-500' : ''} size={20} />
                    </button>
            ...
            {quizSubmitted ? (
              <div className={`p-8 rounded-2xl border text-center ${
                ...
                <h2 className="text-3xl font-bold mb-2">
                  {quizScore >= topic.quiz.passingScore ? 'Quiz Passed!' : 'Needs More Revision!'}
                </h2>
              ...
              <>
                <div className="bg-blue-900/20 border border-blue-500/30 p-6 rounded-2xl mb-8">
                  ...
                  <p className="text-blue-200">Validate your knowledge. Passing score: {topic.quiz.passingScore}%</p>
                </div>
                      ...
                      {q.options.map((opt, oIndex) => (
                        ...
                        >
                          ...
                          <div className={`w-5 h-5 rounded-full border-2 mr-4 flex items-center justify-center ${
                            quizAnswers[qIndex] === oIndex ? 'border-blue-500' : 'border-gray-500'
                          }`}>
                            {quizAnswers[qIndex] === oIndex && <div className="w-2.5 h-2.5 rounded-full bg-blue-500" />}
                          </div>
                          <span>{opt}</span>
                        </label>
                      ))}
                ...
                <button 
                  ...
                  className="w-full py-4 bg-blue-600 hover:bg-blue-500 disabled:bg-gray-800 disabled:text-gray-500 rounded-xl font-bold transition-colors"
                >
                  Submit Quiz
                </button>
              </>
