...
export default function AdminOSTopicEditor() {
  const { id } = useParams();
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      if (activeTab === 'notes') {
        const res = await axios.get(`/api/admin/os/topic/${id}/notes`, { withCredentials: true });
        setNotesContent(res.data.content || '');
      } else if (activeTab === 'questions') {
        const res = await axios.get(`/api/admin/os/topic/${id}/questions`, { withCredentials: true });
        setQuestions(res.data);
      } else if (activeTab === 'quiz') {
        const res = await axios.get(`/api/admin/os/topic/${id}/quiz`, { withCredentials: true });
  ...
  const handleSaveNotes = async () => {
    try {
      setSaving(true);
      await axios.post(`/api/admin/os/topic/${id}/notes`, { content: notesContent, sections: [] }, { withCredentials: true });
  ...
  const handleAddQuestion = async (e) => {
    ...
    try {
      ...
      await axios.post(`/api/admin/os/topic/${id}/question`, {
        ...newQuestion,
        companyTags: tags
  ...
  const handleDeleteQuestion = async (qId) => {
    ...
    try {
      await axios.delete(`/api/admin/os/question/${qId}`, { withCredentials: true });
      fetchData();
    } catch (error) {
  ...
  const handleSaveQuiz = async () => {
    try {
      setSaving(true);
      await axios.post(`/api/admin/os/topic/${id}/quiz`, quiz, { withCredentials: true });
      ...
      <div className="flex gap-4 border-b border-white/10 mb-8">
        <button
          ...
          className={`pb-4 px-4 font-medium transition-colors border-b-2 ${
            activeTab === 'notes' ? 'border-emerald-500 text-emerald-400' : 'border-transparent text-gray-400 hover:text-white'
          }`}
        >
          OS Notes
        </button>
        <button
          ...
          className={`pb-4 px-4 font-medium transition-colors border-b-2 ${
            activeTab === 'questions' ? 'border-emerald-500 text-emerald-400' : 'border-transparent text-gray-400 hover:text-white'
          }`}
        >
          Interview Questions
        </button>
        <button
          ...
          className={`pb-4 px-4 font-medium transition-colors border-b-2 ${
            activeTab === 'quiz' ? 'border-emerald-500 text-emerald-400' : 'border-transparent text-gray-400 hover:text-white'
          }`}
        >
          Practice Quiz
        </button>
            ...
            <div className="flex flex-col">
              <div className="flex justify-between items-center mb-4">
                ...
                <button
                  ...
                  disabled={saving}
                  className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 px-4 py-2 rounded-lg font-semibold transition-colors text-sm"
                >
              ...
              <textarea
                ...
                className="flex-1 w-full bg-[#111] border border-white/10 rounded-xl p-4 focus:outline-none focus:border-emerald-500 font-mono text-sm resize-none"
                placeholder="# Enter markdown content here..."
              />
            </div>
            <div className="flex flex-col">
              <label className="font-semibold text-gray-300 mb-4 block">Live Preview</label>
              <div className="flex-1 bg-[#111] border border-white/10 rounded-xl p-6 overflow-y-auto prose prose-invert prose-emerald max-w-none">
          ...
          <div>
            <div className="flex justify-between items-center mb-6">
              ...
              <button
                ...
                className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 px-4 py-2 rounded-lg font-semibold transition-colors"
              >
                <FiPlus /> Add Question
              </button>
            </div>
              ...
              <form onSubmit={handleAddQuestion} className="bg-[#111] p-6 rounded-xl border border-white/10 mb-8">
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-black/50 border border-white/10 rounded-lg p-2.5 focus:outline-none focus:border-emerald-500 text-white"
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-black/50 border border-white/10 rounded-lg p-2.5 focus:outline-none focus:border-emerald-500"
                      placeholder="e.g., Google, Oracle, Amazon"
                    />
                  </div>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <textarea
                    ...
                    className="w-full bg-black/50 border border-white/10 rounded-lg p-2.5 focus:outline-none focus:border-emerald-500 h-32"
                    required
                  />
                </div>
                ...
                <div className="flex justify-end gap-3">
                  ...
                  <button
                    type="submit"
                    className="bg-emerald-600 hover:bg-emerald-500 px-6 py-2 rounded-lg font-semibold transition-colors"
                  >
                    Save Question
                  </button>
                </div>
        ...
        {activeTab === 'quiz' && (
          <div className="max-w-3xl">
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#111] border border-white/10 rounded-lg p-2.5 focus:outline-none focus:border-emerald-500"
                />
              </div>
              <div>
                ...
                <div key={qIndex} className="bg-[#111] p-6 rounded-xl border border-white/10 relative">
                  ...
                  <div className="mb-4 pr-8">
                    ...
                    <input
                      ...
                      className="w-full bg-black/50 border border-white/10 rounded-lg p-2 focus:outline-none focus:border-emerald-500"
                    />
                  </div>
                      ...
                      <div key={oIndex} className="flex items-center gap-3">
                        ...
                        <input
                          ...
                          className={`flex-1 bg-black/50 border rounded-lg p-2 focus:outline-none ${
                            q.correctAnswerIndex === oIndex ? 'border-emerald-500' : 'border-white/10 focus:border-gray-500'
                          }`}
            ...
            <button
              ...
              className="mt-6 w-full py-4 border-2 border-dashed border-white/20 hover:border-emerald-500 text-gray-400 hover:text-emerald-400 rounded-xl transition-colors font-semibold flex items-center justify-center gap-2"
            >
