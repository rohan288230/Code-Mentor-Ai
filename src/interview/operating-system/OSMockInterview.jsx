...
export default function OSMockInterview() {
  ...
  const handleComplete = async (finalPoints = 0) => {
    ...
    try {
      await axios.post('/api/interview/os/mock', {
        score: percentage,
        feedback: `Completed mock interview at ${difficulty} difficulty.`
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] text-white flex items-center justify-center p-6">
        <div className="bg-[#111] border border-white/10 rounded-2xl p-8 max-w-lg w-full text-center">
          <FiCheckSquare className="text-6xl text-emerald-500 mx-auto mb-6" />
          ...
          <div className="bg-black/50 p-6 rounded-xl border border-white/5 mb-8">
            <div className="text-sm text-gray-500 mb-2">Your OS Score</div>
            <div className={`text-5xl font-bold ${score >= 70 ? 'text-emerald-500' : 'text-yellow-500'}`}>
              {score}%
            </div>
          </div>
          ...
          <div className="flex gap-4 justify-center">
            ...
            <button 
              ...
              className="px-6 py-3 bg-emerald-600 hover:bg-emerald-500 rounded-xl transition-colors font-semibold"
            >
              Try Again
            </button>
          </div>
      ...
      <div className="min-h-screen bg-[#0a0a0a] text-white">
        ...
        <div className="sticky top-0 bg-[#0a0a0a]/80 backdrop-blur-md border-b border-white/10 z-40">
          <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-emerald-400 font-mono text-xl bg-emerald-500/10 px-4 py-1.5 rounded-lg border border-emerald-500/20">
              <FiClock />
              {formatTime(timeLeft)}
            </div>
          </div>
            ...
            <div className="bg-[#111] border border-white/10 rounded-2xl p-8">
              ...
              {!showAnswer ? (
                <button 
                  ...
                  className="w-full py-4 border-2 border-dashed border-gray-600 hover:border-emerald-500 text-gray-400 hover:text-emerald-400 rounded-xl transition-colors font-semibold"
                >
                ...
                <div className="space-y-6 animate-fadeIn">
                  <div className="bg-black/50 p-6 rounded-xl border border-white/5">
                    <h4 className="text-emerald-400 font-semibold mb-3">Expected Answer:</h4>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      <button onClick={() => handleNext(10)} className="py-3 bg-emerald-900/30 hover:bg-emerald-900/50 border border-emerald-500/30 rounded-xl text-emerald-400 font-semibold transition-colors">
                        Nailed it! (10)
                      </button>
                    </div>
                  </div>
          ...
          <div className="bg-[#111] border border-white/10 rounded-2xl flex flex-col h-[600px] overflow-hidden relative">
            <div className="p-4 border-b border-white/10 flex justify-between items-center bg-black/30">
              <h3 className="font-bold flex items-center gap-2">
                <FiEdit2 className="text-emerald-500" /> OS Whiteboard
              </h3>
            ...
            {showWhiteboard ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-gray-500 p-8 text-center">
                <FiTerminal size={48} className="mb-4 opacity-50 text-emerald-500" />
                <p>Use the whiteboard to sketch out your memory layouts, scheduling graphs, or OS architectures.</p>
                <button 
                  onClick={() => setShowWhiteboard(true)}
                  className="mt-4 px-4 py-2 bg-emerald-600/20 text-emerald-400 rounded-lg hover:bg-emerald-600/30 transition-colors"
                >
                  Open Whiteboard
                </button>
              </div>
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white flex items-center justify-center p-6">
      ...
      <div className="max-w-md w-full">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 mb-6 shadow-lg shadow-emerald-500/20">
            <FiPlayCircle size={32} className="text-white ml-1" />
          </div>
          <h1 className="text-3xl font-bold mb-3">OS Mock Interview</h1>
          <p className="text-gray-400">Simulate a real operating systems interview with whiteboard support.</p>
        </div>
        ...
        <div className="bg-[#111] border border-white/10 rounded-2xl p-6 mb-6">
          ...
          <div className="grid grid-cols-1 gap-3">
            {['Beginner', 'Medium', 'Advanced'].map(diff => (
              ...
              >
                <span className={`font-semibold ${difficulty === diff ? 'text-emerald-400' : 'text-gray-300'}`}>{diff}</span>
                {difficulty === diff && <FiCheckSquare className="text-emerald-500" />}
              </button>
            ))}
          </div>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-4 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 rounded-xl font-bold text-lg shadow-lg shadow-emerald-500/25 transition-all flex justify-center items-center gap-2"
        >
          {loading ? (
