    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] text-white flex items-center justify-center p-6">
        <div className="bg-[#111] border border-white/10 rounded-2xl p-8 max-w-lg w-full text-center">
          <FiCheckSquare className="text-6xl text-green-500 mx-auto mb-6" />
          ...
          <div className="bg-black/50 p-6 rounded-xl border border-white/5 mb-8">
            <div className="text-sm text-gray-500 mb-2">Your Architecture Score</div>
            <div className={`text-5xl font-bold ${score >= 70 ? 'text-green-500' : 'text-yellow-500'}`}>
              {score}%
            </div>
          </div>
          ...
          <div className="flex gap-4 justify-center">
            ...
            <button 
              ...
              className="px-6 py-3 bg-green-600 hover:bg-green-500 rounded-xl transition-colors font-semibold"
            >
              Try Again
            </button>
          </div>
      ...
      <div className="min-h-screen bg-[#0a0a0a] text-white">
        ...
        <div className="sticky top-0 bg-[#0a0a0a]/80 backdrop-blur-md border-b border-white/10 z-40">
          <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-green-400 font-mono text-xl bg-green-500/10 px-4 py-1.5 rounded-lg border border-green-500/20">
              <FiClock />
              {formatTime(timeLeft)}
            </div>
          </div>
            ...
            <div className="bg-[#111] border border-white/10 rounded-2xl p-8">
              ...
              {!showAnswer ? (
                <button 
                  ...
                  className="w-full py-4 border-2 border-dashed border-gray-600 hover:border-green-500 text-gray-400 hover:text-green-400 rounded-xl transition-colors font-semibold"
                >
                  Reveal Expected Architecture & Answer
                </button>
              ) : (
                <div className="space-y-6 animate-fadeIn">
                  <div className="bg-black/50 p-6 rounded-xl border border-white/5">
                    <h4 className="text-green-400 font-semibold mb-3">Expected Answer:</h4>
                    <p className="text-gray-300 whitespace-pre-wrap leading-relaxed">{q.answer}</p>
                  </div>
                  
                  <div>
                    <h4 className="font-semibold mb-3 text-gray-300">Self Evaluation: How well did you design this?</h4>
          ...
          <div className="bg-[#111] border border-white/10 rounded-2xl flex flex-col h-[600px] overflow-hidden relative">
            <div className="p-4 border-b border-white/10 flex justify-between items-center bg-black/30">
              <h3 className="font-bold flex items-center gap-2">
                <FiEdit2 className="text-green-500" /> System Design Whiteboard
              </h3>
            ...
            {showWhiteboard ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-gray-500 p-8 text-center">
                ...
                <p>Use the whiteboard to sketch out your architecture diagrams, draw load balancers, databases, and caches.</p>
                <button 
                  onClick={() => setShowWhiteboard(true)}
                  className="mt-4 px-4 py-2 bg-green-600/20 text-green-400 rounded-lg"
                >
                  Open Whiteboard
                </button>
              </div>
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white flex items-center justify-center p-6">
      ...
      <div className="max-w-md w-full">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-green-500 to-teal-600 mb-6 shadow-lg shadow-green-500/20">
            <FiPlayCircle size={32} className="text-white ml-1" />
          </div>
          <h1 className="text-3xl font-bold mb-3">System Design Mock Interview</h1>
          <p className="text-gray-400">Simulate a real software engineering system design interview with whiteboard support.</p>
        </div>
        ...
        <div className="bg-[#111] border border-white/10 rounded-2xl p-6 mb-6">
          ...
          <div className="grid grid-cols-1 gap-3">
            {['Beginner', 'Medium', 'Advanced'].map(diff => (
              ...
              >
                <span className={`font-semibold ${difficulty === diff ? 'text-green-400' : 'text-gray-300'}`}>{diff}</span>
                {difficulty === diff && <FiCheckCircle className="text-green-500" />}
              </button>
            ))}
          </div>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-4 bg-gradient-to-r from-green-600 to-teal-600 hover:from-green-500 hover:to-teal-500 rounded-xl font-bold text-lg shadow-lg shadow-green-500/25 transition-all flex justify-center items-center gap-2"
        >
          {loading ? (
