...
import { FiPlus, FiEdit2, FiTrash2, FiTerminal } from 'react-icons/fi';
...
export default function AdminOSDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error('Error fetching admin OS data:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddTopic = async (e) => {
    ...
    try {
      await axios.post('/api/admin/os/topic', {
        ...newTopic,
  ...
  const handleDeleteTopic = async (id) => {
    ...
    try {
      await axios.delete(`/api/admin/os/topic/${id}`, { withCredentials: true });
      fetchData();
    } catch (error) {
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-3">
            <FiTerminal className="text-emerald-500" /> OS Editor
          </h1>
          <p className="text-gray-400 mt-2">Manage operating system topics, interview questions, and quizzes.</p>
        </div>
        <button
          onClick={() => setIsAddingTopic(true)}
          className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-lg font-semibold transition-colors"
        >
          <FiPlus /> Add Topic
        </button>
      </div>
          ...
          <form onSubmit={handleAddTopic} className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-black/50 border border-white/10 rounded-lg p-2.5 focus:outline-none focus:border-emerald-500 h-24"
                required
              />
            </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {topics.map(topic => (
          <div key={topic._id} className="bg-[#111] border border-white/5 p-6 rounded-xl flex items-center justify-between group hover:border-white/20 transition-all">
            ...
            <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
              <button 
                onClick={() => navigate(`/admin/os/topic/${topic._id}`)}
                className="p-3 bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 rounded-lg transition-colors"
                title="Edit Content"
              >
                <FiEdit2 />
              </button>
        ...
        {topics.length === 0 && !isAddingTopic && (
          <div className="text-center py-12 bg-[#111] rounded-xl border border-white/5">
            <FiTerminal className="text-4xl text-gray-600 mx-auto mb-3" />
