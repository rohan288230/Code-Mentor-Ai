  ...
  if (!subject) {
    return (
      <div className="flex justify-center items-center h-screen bg-[#0a0a0a] text-white">
        <h2>System Design content not found.</h2>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white p-6 pb-24">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        {/* Header Section */}
        <div className="bg-gradient-to-r from-green-900/40 to-teal-900/40 rounded-3xl p-8 border border-white/10 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-12 opacity-10">
            <FiServer size={120} />
          </div>
          <div className="relative z-10">
            <div className="flex items-center gap-3 text-green-400 mb-4 font-semibold tracking-wider text-sm uppercase">
              <FiBookOpen />
            ...
            <div className="flex flex-wrap gap-4 items-center">
              <button
                onClick={() => navigate('/interview/system-design/mock')}
                className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-green-600 to-teal-600 hover:from-green-500 hover:to-teal-500 rounded-xl font-semibold transition-all shadow-[0_0_20px_rgba(16,185,129,0.3)]"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {subject.topics.map((topic, index) => {
              ...
              return (
                <div 
                  key={topic._id}
                  onClick={() => navigate(`/interview/system-design/topic/${topic._id}`)}
                  className="group bg-[#111] border border-white/5 hover:border-green-500/50 rounded-2xl p-6 cursor-pointer transition-all hover:transform hover:-translate-y-1 relative overflow-hidden"
                >
                  <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-green-500 to-teal-500 opacity-0 group-hover:opacity-100 transition-opacity"></div>
